    ...
    extends RcdScriptBean
{
        ...
        return runSafely( () -> {
            NodeEditor nodeEditor = ( editableNode ) -> {
                ...
                if ("PropertySet".equals( type ) ) {
                    parent.addSet(property.getName());
                } else {
    ...
    public String delete( final String repositoryName, final String branchName, final String path, final String parentPath,
                          final Map<String, Collection<Integer>> properties )
    {
        return runSafely( () -> {
            NodeEditor nodeEditor = ( editableNode ) -> {
                final PropertySet parentPropertySet =
                ...
                properties.forEach((propertyName, indexes) -> {
                    indexes.forEach(index -> {
                        parentPropertySet.removeProperty( propertyName + "[" + index + "]" );
                    });
                });
            };
            final UpdateNodeParams updateNodeParams = UpdateNodeParams.create().
