class ExportsRoute extends DtbRoute {
    ...
    createLayout() {
        this.tableCard = new RcdMaterialTableCard('Node exports').init()
            ...
            .addIconArea(new RcdGoogleMaterialIconArea('file_download', () => this.dowloadExports()).init().setTooltip('Archive and download selected node exports'), {min: 1, max: 1})
            ...
            .then((result) => {
                result.success.sort((export1, export2) => export2.timestamp - export1.timestamp).forEach((anExport) => {
                    this.tableCard.createRow()
                        ...
                        .addCell(toLocalDateTimeFormat(new Date(anExport.timestamp)) + (anExport.size >= 0 ? '\n' + toHumanReadableSize(anExport.size) : ''), {classes: ['non-mobile-cell']}).setAttribute('export', anExport.name);
                });
            })
